...
# === Load data and shift time ===
raw_df = pd.read_csv('TESS_raw.csv')
raw_df['#time'] = raw_df['#time'] - 2457000  # Shift to BJD - 2457000

detrended_df = pd.read_csv('TESS.csv')
detrended_df['#time'] = detrended_df['#time'] - 2457000  # Shift to BJD - 2457000

# === Define time ranges (in BJD - 2457000) ===
time_ranges = [
    (2715, 2770),
    (2910, 2963),
    (3285, 3315),
...
for ax, (t_start, t_end) in zip(axes, time_ranges):
    ...
    # Plot raw data
    ax.plot(raw_slice['#time'], raw_slice['flux'], 'o',
            color=raw_color, markersize=1, alpha=1, label='Raw Data (TESS-SPOC)')

    # Plot detrended data
    ax.plot(detrended_slice['#time'], detrended_slice['flux'], 'o',
            color=detrended_color, markersize=0.5, alpha=0.85, label='Detrended Data (TESS-SPOC)')

    ax.set_xlim(t_start, t_end)
    ax.tick_params(axis='both', which='major', labelsize=20)

# === Axis labels and legend ===
axes[-1].set_xlabel('Time (BJD - 2457000)', fontsize=25)
axes[2].set_ylabel('Relative Flux', fontsize=25)
axes[0].legend(loc='lower center', fontsize=15)
